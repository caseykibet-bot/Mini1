...
case 'song': {
    ...
    async function compressAudio(inputPath, outputPath, targetSizeMB = TARGET_SIZE_MB) {
        try {
            ...
            const constrainedBitrate = Math.min(Math.max(targetBitrate, 64), 192); // Increased bitrate range
            
            await execPromise(
                `ffmpeg -i "${inputPath}" -b:a ${constrainedBitrate}k -ac 2 -ar 44100 -vn -y "${outputPath}"`
            );
            return true;
        } catch (error) {
...
`;
        ...
        const cleanTitle = videoInfo.title.replace(/[^\w\s.-]/gi, '').replace(/\.\./g, '').substring(0, 30);
        ...
        try {
            const response = await fetch(downloadLink, { signal: controller.signal });
            if (!response.ok) {
        ...
        if (fileSizeMB > MAX_FILE_SIZE_MB) {
            ...
            if (compressionSuccess) {
                ...
                compressedFilePath = ''; // Prevent double cleanup
            }
        }

        // Send the audio file using file path (more efficient)
        await socket.sendMessage(sender, {
        ...
        // Cleanup
        await cleanupFiles(finalFilePath);
        ...
        if (err.name === 'AbortError') {
            errorMessage = "*⏰ Download took too long, darling! Try a shorter song?*";
        } else if (err.message.includes('network') || err.message.includes('fetch')) {
            errorMessage = "*🌐 Network issue, love! Check your connection?*";
        } else if (err.message.includes('download') || err.message.includes('link')) {
            errorMessage = "*🔗 Download service error! Try another song?*";
        }
