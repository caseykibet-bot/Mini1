...
💡 *Tip:* Use *.video to get the video version*
        ...
        // Get download link with proper error handling
        let downloadLink;
        try {
            ...
            downloadLink = data.downloadLink || data.url || data.link;
            ...
            return await socket.sendMessage(sender, {
                text: '*❌ Download Service Unavailable*\nThe audio service is currently unavailable. Please try again later.*'
            }, { quoted: msg });
        }

        // Send audio file with timeout
        try {
            await socket.sendMessage(sender, {
                ...
                quoted: msg,
                uploadTimeoutMs: 60000 // 60 second timeout for upload
            });
        } catch (uploadError) {
            console.error('[PLAY] Upload Error:', uploadError.message);
            await socket.sendMessage(sender, {
                text: `*❌ Upload Failed*\nFailed to send the audio file. You can try downloading it directly:\n${downloadLink}`
            }, { quoted: msg });
        }
